...
const AchievementCard: React.FC<Props> = ({ type, data }) => {
      ...
      case 'certificate':
        ...
        return (
          <>
            ...
            <h4 className="text-xl font-medium mb-2 text-white break-words">{cert.title}</h4>
            <p className="text-[var(--white-icon)] mb-1">Issued by {cert.issuer}</p>
          ...
          <>
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-xl font-medium mb-1 text-white break-words">{hack.name}</h4>
                <p className="text-[var(--sec)] font-medium">{hack.position}</p>
              </div>
            </div>
            
            <p className="text-[var(--white-icon)] mb-3 break-words">{hack.description}</p>
        ...
        return (
          <>
            <div className="flex items-start gap-4 mb-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className="text-xl font-medium mb-1 text-white break-words">{workshop.title}</h4>
                <p className="text-[var(--sec)] font-medium">{workshop.role}</p>
              </div>
            </div>
            
            <p className="text-[var(--white-icon)] mb-3 break-words">{workshop.description}</p>
        ...
        return (
          <div className="flex flex-col md:flex-row gap-6">
              ...
              {cert.additionalImages && cert.additionalImages.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {cert.additionalImages.map((img, index) => (
                    <div key={index} className="rounded-lg overflow-hidden bg-[#0a0a0a] aspect-video flex items-center justify-center">
                      <img src={img} alt={`Additional ${index}`} className="max-h-full max-w-full object-contain" />
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="w-full md:w-3/5">
              <h3 className="text-2xl font-medium mb-2 text-white">{cert.title}</h3>
              <p className="text-[var(--sec)] font-medium mb-4">Issued by {cert.issuer} • {cert.date}</p>
              
              {cert.description && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-2 text-white">About this certification</h4>
              ...
              {cert.skills && cert.skills.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-2 text-white">Skills</h4>
                  <div className="flex flex-wrap gap-2">
                    {cert.skills.map((skill, index) => (
                      <span key={index} className="px-2 py-1 text-sm bg-[#ffffff10] rounded-md text-[var(--white-icon)]">{skill}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="flex flex-col md:flex-row gap-6">
              ...
              {hack.additionalImages && hack.additionalImages.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {hack.additionalImages.map((img, index) => (
                    <div key={index} className="rounded-lg overflow-hidden bg-[#0a0a0a] aspect-video flex items-center justify-center">
                      <img src={img} alt={`Additional ${index}`} className="max-h-full max-w-full object-cover" />
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="w-full md:w-3/5">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h3 className="text-2xl font-medium mb-1 text-white">{hack.name}</h3>
                  <p className="text-[var(--sec)] font-medium">{hack.position} • {hack.date}</p>
                </div>
              </div>
              
              {hack.company && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-1 text-white">Company</h4>
              ...
              {hack.problemDescription && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-1 text-white">Challenge</h4>
                  <p className="text-[var(--white-icon)]">{hack.problemDescription}</p>
                </div>
              )}
              
              {hack.techStack && hack.techStack.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-1 text-white">Tech Stack</h4>
                  <div className="flex flex-wrap gap-2">
                    {hack.techStack.map((tech, index) => (
                      <span key={index} className="px-2 py-1 text-sm bg-[#ffffff10] rounded-md text-[var(--white-icon)]">{tech}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {hack.projectLink && (
                <div className="mt-6">
                  <a 
                    ...
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[var(--sec)] text-white font-medium transition-all hover:bg-opacity-90 w-fit"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" width="1.2em" height="1.2em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path>
                      <polyline points="15 3 21 3 21 9"></polyline>
        ...
        return (
          <div className="flex flex-col md:flex-row gap-6">
            <div className="w-full md:w-2/5">
              ...
              {workshop.additionalImages && workshop.additionalImages.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
            ...
            <div className="w-full md:w-3/5">
              <h3 className="text-2xl font-medium mb-1 text-white">{workshop.title}</h3>
              <p className="text-[var(--sec)] font-medium mb-4">{workshop.role} • {workshop.date}</p>
              
              <p className="text-[var(--white-icon)] mb-4">{workshop.description}</p>
              
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <h4 className="text-lg font-medium mb-1 text-white">Organizer</h4>
                  <p className="text-[var(--white-icon)]">{workshop.organizer}</p>
                </div>
                
                {workshop.attendees && (
                  <div>
                    <h4 className="text-lg font-medium mb-1 text-white">Attendees</h4>
                    <p className="text-[var(--white-icon)]">{workshop.attendees}+</p>
                  </div>
                )}
              </div>
              ...
              {workshop.topics && workshop.topics.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-1 text-white">Topics Covered</h4>
                  <div className="flex flex-wrap gap-2">
                    {workshop.topics.map((topic, index) => (
                      <span key={index} className="px-2 py-1 text-sm bg-[#ffffff10] rounded-md text-[var(--white-icon)]">{topic}</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              {workshop.feedback && (
                <div className="mb-4">
                  <h4 className="text-lg font-medium mb-1 text-white">Feedback</h4>
                  <p className="text-[var(--white-icon)] italic">"{workshop.feedback}"</p>
                </div>
              )}
  ...
  return (
    <>
      <div 
        className="achievement-card relative overflow-hidden rounded-xl border border-[var(--white-icon-tr)] bg-[#1414149c] p-5 h-full w-full"
...
></div>
      ...
      {/* Modal - Added portal div to ensure it renders correctly */}
      {showModal && (
        <div 
